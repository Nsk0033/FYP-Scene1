...
#define VOLUMETRIC_MAX_HENYEY_GREENSTEIN 5.0
		...
		Pass
		{
			...
			fixed4 frag(wm_full_screen_fragment i) : SV_Target
			{
				...
				float depth01 = len * GetDepth01(i.uv.xy) * atmosphere01;
		...
		Pass
		{
			...
			fixed4 frag(wm_full_screen_fragment i) : SV_Target
			{
				...
				float shadow = max(0.65, ComputeAtmosphericLightShafts(i.vertex.xy, WEATHER_MAKER_CAMERA_POS_NO_ORIGIN_OFFSET, GetFullScreenRayDir(i.forwardLine), rayLength));
				shadow *= shadow;
