...
fixed4 ComputeSkySphereColorFullScreen(float3 rayDir, float2 uv, float depth01, bool doMie, bool doNight)
{
	...
	if (depthFade > 0.00001)
	{
		...
		if (WM_ENABLE_PROCEDURAL_TEXTURED_SKY || WM_ENABLE_PROCEDURAL_SKY || WM_ENABLE_PROCEDURAL_SKY_ATMOSPHERE || WM_ENABLE_PROCEDURAL_TEXTURED_SKY_ATMOSPHERE)
		{
			...
			else if (_WeatherMakerNightMultiplier == 1.0)
			{
				skyColor = fixed4Zero;
			}
			else
			{
				...
				skyColor.rgb *= _WeatherMakerSkyAtmosphereParams.z; // exposure
				*/
...
fixed4 ComputeSkySphereColorVolumetric(wm_volumetric_data i, bool doMie, bool doNight)
{
	...
	return ComputeSkySphereColorFullScreen(rayDir, i.uv.xy, depth01, doMie, true);
}
